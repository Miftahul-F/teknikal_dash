...
def compute_confidence(ticker):
    try:
        ...
        return {
            ...
            "Last Price": round(df_daily['Close'].iloc[-1], 2)
        }
...
if results:
    ...
    df_results = df_results.sort_values("Confidence", ascending=False).reset_index(drop=True)

    # Tambahkan tanda ✅ jika Confidence >= 90
    df_results["Signal"] = df_results["Confidence"].apply(lambda x: "✅ BUY" if x >= 90 else "-")
